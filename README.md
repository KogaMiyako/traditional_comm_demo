...
以下命令均在 `traditional_comm_py` 目录下执行。项目不需要 Node.js 或前端构建工具，主要依赖 Python 3.12 和 FFmpeg。
...
```powershell
conda activate semcom-py
```

如果不想激活环境，后文命令中的 Python 也可以直接使用：

```text
D:\miniconda3\envs\semcom-py\python.exe
```

检查 Python 和 FFmpeg：

```powershell
& D:\miniconda3\Scripts\conda.exe run -n semcom-py python --version
...
`demo` 会自动生成文字、图片和视频样本，然后使用本地回环链路完成三种媒体类型的测试：
...
打开两个 PowerShell 窗口。两个窗口都先进入 `traditional_comm_py` 目录并使用 `semcom-py` 环境。
...
其中 `5000` 是模拟的业务 TCP 端口；`--max-connections 0` 表示接收端持续监听。只接收一次业务载荷时可以去掉该参数。
...
如果要在一台电脑上模拟两台电脑，需要启动两个 Web 服务，并使用不同的 Web 端口。先生成样本：
...
& D:\miniconda3\envs\semcom-py\python.exe -m traditional_comm.web_server `
    --host 127.0.0.1 `
    --port 8000 `
    --runs runs_receiver_control `
    --receiver-runs runs_receiver
```

打开 `http://127.0.0.1:8000/`，在“接收端控制框”中填写监听地址 `127.0.0.1`、端口 `5000`，点击“启动接收”。
...
& D:\miniconda3\envs\semcom-py\python.exe -m traditional_comm.web_server `
    ...
    --port 8001 `
    --runs runs_sender `
    --receiver-runs runs_sender_receiver
```

打开 `http://127.0.0.1:8001/`，选择样本和“局域网 TCP”，填写接收端 IP `127.0.0.1`、端口 `5000`，点击“开始发送”。
...
打开接收服务器的 `http://<接收服务器IP>:8000/`，在“接收端控制框”中填写 `0.0.0.0` 和 `5000`，点击“启动接收”。打开发送服务器的 Web 页面，把传输方式改为“局域网 TCP”，接收端 IP 填接收服务器局域网地址，端口填 `5000`，再点击“开始发送”。两台电脑的 Web 服务都使用 `8000` 没问题，因为它们运行在不同电脑上。
